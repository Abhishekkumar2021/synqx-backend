...
Revision ID: V1
Revises: 
Create Date: 2025-12-06 11:59:03.146097
...
def upgrade() -> None:
    ...
    sa.UniqueConstraint('tenant_id', 'name', name='uq_alert_config_name_per_tenant')
    )
    ...
    sa.Column('deleted_by', sa.String(length=255), nullable=True),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('tenant_id', 'name', name='uq_connection_name_per_tenant')
    )
    op.create_index('idx_connection_health', 'connections', ['health_status', 'tenant_id'], unique=False)
    op.create_index('idx_connection_type_tenant', 'connections', ['connector_type', 'tenant_id'], unique=False)
    ...
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('tenant_id', 'name', name='uq_pipeline_name_per_tenant')
    )
    op.create_index('idx_pipeline_schedule', 'pipelines', ['schedule_enabled', 'schedule_cron'], unique=False)
    op.create_index('idx_pipeline_status_tenant', 'pipelines', ['status', 'tenant_id'], unique=False)
    ...
    op.create_index('idx_asset_type_tenant', 'assets', ['asset_type', 'tenant_id'], unique=False)
    ...
    sa.Column('name', sa.String(length=255), nullable=False),
    sa.Column('description', sa.Text(), nullable=True),
    sa.Column('operator_type', sa.Enum('EXTRACT', 'TRANSFORM', 'LOAD', 'VALIDATE', 'TRIGGER', 'NOOP', name='operatortype'), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index('idx_pipeline_status_tenant', table_name='pipelines')
    ...
    op.drop_index('idx_connection_type_tenant', table_name='connections')
    op.drop_index('idx_connection_health', table_name='connections')
